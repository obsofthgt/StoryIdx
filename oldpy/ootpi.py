...
def build_final_html(pages):
    ...
    if CONFIG['ARW_DISPLAY']:
        # Show blocks and arrows only on hover
        html_template = HTML_TEMPLATE\
            .replace('ARW_DISPLAY', 'flex')\
            .replace('ARW_VISIBILITY', 'visible')\
            .replace('ARW_OPACITY', '0')  # Completely transparent when not hovering
            .replace('ARW_HOVER_OPACITY', '1')  # Fully visible when hovering
    else:
